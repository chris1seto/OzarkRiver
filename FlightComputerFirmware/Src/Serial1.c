...
static const char* TAG = "SERIAL1";

static void Serial1Deinit(void);
static bool Serial1Init(const uint32_t baud, const uint32_t new_data_rx_event_count);
static void CopyFromDma(void);
static void Serial1Task(void* arg);
...
SerialInterface_t* Serial1_GetInterface(void)
{
...
static bool Serial1Init(const uint32_t baud, const uint32_t new_data_rx_event_count)
{
  ...
  /*
    USART1 DMA2
      RX: (Stream 2, Channel 4), (Stream 5, channel 4), 
      TX: (Stream 7, channel 4), 
  */

  // DMA RX
  dma_rx_handle.PeriphOrM2MSrcAddress = LL_USART_DMA_GetRegAddr(USART1);
  ...
  xTaskCreate(Serial1Task, TAG, 500, NULL, 0, NULL);
...
static void CopyFromDma(void)
{
  ...
  end_index = DMA_BUFFER_SIZE - LL_DMA_GetDataLength(DMA2, LL_DMA_STREAM_2);
